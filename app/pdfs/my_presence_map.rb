class MyPresenceMap < PdfApplication
  include Prawn::View
  ...
  def initialize(current, params, view_context)
    @margin_top = 137
    @presence_colors = {"F" => 'ff0000', "A" => 'fab230', "S" => 'fab230', "V" => '0000ff', nil => '00b300'}
    ...
    employee_ids = current.employee.efective_position&.position&.positions&.includes(:efective)&.map(&:efective_id) || [current.employee.id]
    @employees = Employee.where(id: employee_ids).employee_id(params[:employee]).includes(:user).order(:number)
    ...
    EmployeeDelay.where(employee_id: current.employee.id).each{|e| @absences[[e.employee_id, e.arrived_at.day, e.arrived_at.month, e.arrived_at.year]] ||= "A#{e&.has_justification? ? e&.letter : ""}"} if (params[:kind].blank? || params[:kind].includes?('delays', 'all'))
    EmployeeExit.where(employee_id: current.employee.id).index_by{|e| @absences[[e.employee_id, e.date.day, e.date.month, e.date.year]] ||= "S#{e&.has_justification? ? e&.letter : ""}" } if (params[:kind].blank? || params[:kind].includes?('exits', 'all'))
    EmployeePresence.where(employee_id: current.employee.id).each{|f| @absences[[f.employee_id, f.date.day, f.date.month, f.date.year]] ||= "P-#{f.status}" } if params[:kind].blank? || params[:kind].includes?('presences', 'all')
    VacationDay.all.includes(:employee_vacation).select{|v| v.approved? || v.pending?}.each{|e| @absences[[e.employee_vacation.employee_id, e.date.day, e.date.month, e.date.year]] ||= "V#{e.employee_vacation.letter}" unless e.employee_vacation_id == @employee.id} if params[:kind].blank? || params[:kind].includes?('vacations', 'all')
