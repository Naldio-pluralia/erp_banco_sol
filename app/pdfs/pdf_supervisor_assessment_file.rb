class PdfSupervisorAssessmentFile < PdfApplication
  include Prawn::View
  ...
  def initialize(employee, assessment, view_context)
    super(top_margin: 327, bottom_margin: 100)
    @employee = employee
    ...
    @supervisor_position = @position&.position
    @supervisor_employee = @supervisor_position&.efective
    @supervisor_department = @supervisor_position&.department
    @supervisor_function = @supervisor_position&.function
  ...
  def header
    ...
    page_count.times do |i|
      ...
      bounding_box([0, bounds.height + 295], :width => 540, :height => 72) do
        # stroke_bounds
      ...
      table([["#{pdf_t("supervised_assessment_file")}"]], position: 0, :column_widths => [540], :cell_style =>  {:borders => [], align: :center, :size => 9, :font_style => :bold})

      table([["#{pdf_t("supervised_assessed_id")}"]], position: 0, :column_widths => [540], :cell_style =>  {:borders => [], align: :center, :size => 9, :font_style => :bold})
      ...
      table([["#{pdf_t("supervisor_id")}"]], position: 0, :column_widths => [540], :cell_style =>  {:borders => [], align: :center, :size => 9, :font_style => :bold})

      table([["#{pdf_t("name")}: #{@supervisor_employee&.full_name || I18n.t('n/a')}", "#{pdf_t("employee_number")}: #{@supervisor_employee&.number || I18n.t('n/a')}"]],
        position: 0,
        :column_widths => [400, 140],
        :cell_style =>  {:borders => [], :align => :left, valign: :center, :size => 8,:font_style => :bold}) do
        row(0).style(height: 25)
      ...
      table([["#{pdf_t("function.name")}: #{@supervisor_function&.name || I18n.t('n/a')}", "#{pdf_t("department_name")}: #{@supervisor_department&.name || I18n.t('n/a')}"]],
        position: 0,
        :column_widths => [270, 270],
        :cell_style =>  {:borders => [], :align => :left, valign: :center, :size => 8,:font_style => :bold}) do
        row(0).style(height: 25)
      ...
      table([["#{pdf_t("supervised_assessment_factors")}"]], position: 0, :column_widths => [540], :cell_style =>  {:borders => [], align: :center, :size => 9, :font_style => :bold})

      table([["#{pdf_t("assessment_scale")}"]], position: 0, :column_widths => [540], :cell_style =>  {:borders => [], align: :center, :size => 9, :font_style => :bold})

      table([["#{pdf_t("skill")}", "#{pdf_t("short.supervisor_assessment")}"]],
        position: 0,
      ...
      bounding_box([0, bounds.bottom - 30], :width => bounds.width, :height => 30) do
        table([["#{pdf_t('supervisor_signature')}: ____________________________________", "#{pdf_t('assessment_date')}: ____/_____/________"]],
          position: 0,
  ...
  def employee_goals_list
    ...
    @employee_goals.each do |f|
      if f.supervisor_assessment > 0
        data << [f.skill.name, f.supervisor_assessment]
      else
        data << [f.skill.name, '']
      end
    end
