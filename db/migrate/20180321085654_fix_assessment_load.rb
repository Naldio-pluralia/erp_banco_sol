class FixAssessmentLoad < ActiveRecord::Migration[5.1]
  def change
    ...
    p employee_assessments = EmployeeAssessment.create(assessments.map{|a| {assessment_id: assessment.id, employee_id: a.first, result: a.second, manual: true}})
    employee_assessments.each{|e| EmployeeAssessment.where(employee_id: e.employee_id, assessment_id: e.assessment_id).update_all(result: e.result, manual: true) if e.id.nil?}
  end
end
